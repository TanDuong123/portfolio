...
// ─────────────────────────────────────────────────────────
// Typing Animation
// Mirrors the vanilla JS implementation in code.html 1-to-1:
// mutable refs for the state machine, only setState for display text.
// ─────────────────────────────────────────────────────────
...
function TypingText() {
  ...
  useEffect(() => {
    ...
    // Initial delay before first letter
    timerRef.current = setTimeout(tick, 800);
    return () => { if (timerRef.current) clearTimeout(timerRef.current); };
  }, []); // runs once — same as calling type() in code.html

  return (
    // typing-container::after blink cursor is handled in globals.css
    <span
      className="typing-container text-[#e5e2e3]"
      style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "18px", lineHeight: 1 }}
    >
      {displayText}
    </span>
  );
...
// ─────────────────────────────────────────────────────────
// Terminal Window
// Structure mirrors code.html exactly.
// Uses material-symbols-outlined check_circle for success line.
// ─────────────────────────────────────────────────────────
...
function TerminalWindow() {
  ...
  return (
    /* translate-x-4 translate-y-4 matches code.html exactly */
    <div className="glass-card rounded-xl overflow-hidden shadow-2xl translate-x-4 translate-y-4">
      ...
      <div className="bg-white/5 px-4 py-2 flex items-center gap-2 border-b border-white/5">
        ...
        <span
          className="mx-auto text-white/40 text-[10px]"
      ...
      {/* Body — font-label-mono text-sm space-y-2 as in code.html */}
      <div
        className="p-6 space-y-2 min-h-[192px]"
      ...
      >
          ...
          if (line.type === "cmd") {
            return (
              ...
              >
                ...
                <span className="text-[#e5e2e3]">{line.text}</span>
              </motion.div>
            );
          }
          if (line.type === "info") {
          ...
          if (line.type === "blue") {
            return (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                transition={{ duration: 0.2 }}
                className="text-[#aec6ff]/60"
              >
                {line.text}
              </motion.div>
            );
          }
          // success — material-symbols check_circle, identical to code.html
          return (
            <motion.div
              key={i}
              initial={{ opacity: 0 }}
            ...
            >
              <span className="material-symbols-outlined text-sm leading-none mt-[1px]">
                check_circle
              </span>
              <span>{line.text}</span>
            </motion.div>
        ...
        {visible < TERMINAL_LINES.length && (
          <span
            className="inline-block w-[9px] h-[15px] bg-[#00DFD8]/80 typing-cursor align-text-bottom"
          />
        )}
      </div>
    </div>
...
export default function Hero() {
  ...
  return (
    ...
    >
      {/* Animated grid — opacity-20 from code.html */}
      <div className="absolute inset-0 animated-grid pointer-events-none opacity-20" />

      {/* Electric-Monster interactive canvas */}
      <ElectricMonster />

      {/* Background glow blobs — hero-glow class (blur 120px, opacity 0.15) */}
      ...
      <motion.div
        style={{ y, opacity }}
        className="relative container mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center z-10"
      >
        {/* ── LEFT COLUMN ─────────────────────────────── */}
        <motion.div
          initial="hidden"
        ...
        >
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#00DFD8]/20 bg-[#00DFD8]/5">
              ...
              <span
                className="uppercase tracking-widest text-[#00DFD8] text-xs"
                style={{ fontFamily: "'JetBrains Mono', monospace" }}
              >
          ...
          >
            <h1
              ...
              style={{
                fontSize: "clamp(3.2rem, 8vw, 72px)",
                fontWeight: 700,
          ...
          {/* Typing row — terminal icon + typing text, matches code.html layout */}
          <motion.div
            variants={{
              hidden: { opacity: 0, y: 24 },
              visible: { opacity: 1, y: 0, transition: { duration: 0.7, ease: [0.22, 1, 0.36, 1] } },
          ...
          >
            {/* material-symbols terminal icon, same as code.html */}
            <span className="material-symbols-outlined text-[#00DFD8] text-[20px] leading-none flex-shrink-0">
              terminal
            </span>
            <TypingText />
          </motion.div>
          ...
          >
            <motion.button
              ...
              className="px-8 py-4 rounded-full font-bold tracking-widest uppercase text-[#020203]"
              style={{
                background: "linear-gradient(90deg, #00DFD8, #BD00FF)",
            ...
            <motion.button
              ...
              className="px-8 py-4 rounded-full border border-white/10 hover:border-white/20 tracking-widest text-[#e5e2e3] transition-colors"
        ...
        >
          ...
          {/* Floating card — top right
              code.html: animate-bounce, animation-duration 4s, -top-10 -right-4 */}
          <motion.div
          ...
          >
            <div className="flex justify-between items-center mb-2">
              ...
              <span
                className="text-[#00DFD8] text-[10px] font-bold"
                style={{ fontFamily: "'JetBrains Mono', monospace" }}
              >
                98.2%
              </span>
            ...
            <div className="h-1 bg-white/5 rounded-full overflow-hidden">
              <motion.div
                className="h-full bg-[#00DFD8] rounded-full"
                initial={{ width: 0 }}
                animate={{ width: "98%" }}
          ...
          {/* Floating card — bottom left
              code.html: bottom-10 -left-10, -rotate-3, hover:rotate-0 */}
          <motion.div
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded bg-[#BD00FF]/20 flex items-center justify-center flex-shrink-0">
                <span className="material-symbols-outlined text-[#BD00FF] text-sm leading-none">
                  database
                </span>
              </div>
              <div>
                <div
                  className="text-white/40 text-[10px] uppercase mb-0.5"
                  style={{ fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.05em" }}
                >
                  Query Performance
                </div>
                <div
                  className="text-[#e5e2e3] text-xs font-bold"
                  style={{ fontFamily: "'JetBrains Mono', monospace" }}
                >
                  0.42ms Average
                </div>
